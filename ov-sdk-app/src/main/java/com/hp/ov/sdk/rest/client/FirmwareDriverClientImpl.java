...
public class FirmwareDriverClientImpl implements FirmwareDriverClient
{
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(FirmwareDriverClientImpl.class);
    @Autowired
    ...
    public FwBaseline getFirmwareDriver(final RestParams params, final String resourceId)
    {
        ...
        params.setUrl(sdkUtils.createRestUrl(params.getHostname(),
                ResourceUris.FIRMWARE_DRIVER_URI, resourceId));

        final String returnObj = restClient.sendRequestToHPOV(params, null);
        logger.debug("FirmwareDriverClientImpl : getFirmwareDriver : response from OV :"
                + returnObj);
        if (null == returnObj || returnObj.equals(""))
        {
            throw new SDKNoResponseException(
                    SDKErrorEnum.noResponseFromAppliance, null, null, null,
                    SdkConstants.FIRMWARE_DRIVER, null);
        }
        // Call adaptor to convert to DTO

        final FwBaseline fwBaselineDto = adaptor.buildDto(returnObj);

        logger.debug("FirmwareDriverClientImpl : getFirmwareDriver : name :"
                + fwBaselineDto.getName());
    ...
    public FwBaselineCollection getAllFirmwareDrivers(final RestParams params)
    {
        ...
        params.setType(HttpMethodType.GET);
        params.setUrl(sdkUtils.createRestUrl(params.getHostname(),
                ResourceUris.FIRMWARE_DRIVER_URI));

        final String returnObj = restClient.sendRequestToHPOV(params, null);
        logger.debug("FirmwareDriverClientImpl : getAllFirmwareDrivers : response from OV :"
                + returnObj);
        if (null == returnObj || returnObj.equals(""))
        {
            throw new SDKNoResponseException(
        ...
        logger.debug("FirmwareDriverClientImpl : getAllFirmwareDrivers : count :"
                + fwBaselineCollectionDto.getCount());
            ...
            final String firmwareName)
    {
        ...
        for (final FwBaseline fwBaselineDto : new ArrayList<>(
                fwBaselineCollectionDto.getMembers()))
        {
        ...
        throw new SDKResourceNotFoundException(SDKErrorEnum.invalidArgument,
