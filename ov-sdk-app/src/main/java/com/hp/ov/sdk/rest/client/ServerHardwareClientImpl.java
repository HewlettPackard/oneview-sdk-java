...
public class ServerHardwareClientImpl implements ServerHardwareClient
{
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(ServerHardwareClientImpl.class);
    ...
    public ServerHardware getServerHardware(final RestParams params, final String resourceId)
    {
        ...
        params.setUrl(sdkUtils.createRestUrl(params.getHostname(),
                ResourceUris.SERVER_HARWARE_URI, resourceId));

        final String returnObj = restClient.sendRequestToHPOV(params, null);
        logger.debug("ServerHardwareClient : getServerHardware : response from OV :"
                + returnObj);
        ...
        logger.debug("ServerHardwareClient : getServerHardware : name :"
                + serverHardwareDto.getName());
    ...
    public ServerHardwareCollection getAllServerHardwares(final RestParams params)
    {
        ...
        params.setType(HttpMethodType.GET);
        params.setUrl(sdkUtils.createRestUrl(params.getHostname(),
                ResourceUris.SERVER_HARWARE_URI));

        // call rest client
        final String returnObj = restClient.sendRequestToHPOV(params, null);
        logger.debug("ServerHardwareClientImpl : getAllServerHardwares : response from OV :"
                + returnObj);
        ...
        logger.debug("ServerHardwareClientImpl : getAllServerHardwares : members count :"
            ...
            final RestParams params)
    {
        ...
        // call rest client
        final String returnObj = restClient.sendRequestToHPOV(params, null);
        logger.debug("ServerHardwareClientImpl : getServerHardwareWithNoProfile : response from OV :"
                + returnObj);
        ...
        logger.debug("ServerHardwareClientImpl : getServerHardwareWithNoProfile : members count :"
                + serverHardwareCollectionDto.getCount());
        final List<ServerHardware> tempServerHardwareCollectionDto = serverHardwareCollectionDto
                .getMembers();
        for (final ServerHardware serverHardwareDto : new ArrayList<>(
                serverHardwareCollectionDto.getMembers()))
        {
            if (serverHardwareDto.getServerProfileUri() != null
                    && !serverHardwareDto.getServerProfileUri().isEmpty())
            {
            ...
            final String destinationBay)
    {
        ...
        logger.error("ServerHardwareClientImpl : getServerHardwareByName : resource not Found for name :"
                + destinationBay);
            ...
            final boolean aSync, final boolean useJsonRequest)
    {
        ...
        {
            throw new SDKInvalidArgumentException(SDKErrorEnum.invalidArgument,
                    null, null, null, SdkConstants.POWERSTATE, null);
        }
        // set the additional params
        ...
        jsonObject = adaptor
                .buildJsonObjectFromDto(serverPowerControlRequestDto);
        returnObj = restClient.sendRequestToHPOV(params, jsonObject);
        // convert returnObj to taskResource
        TaskResourceV2 taskResourceV2 = taskAdaptor.buildDto(returnObj);

        logger.debug("ServerHardwareClientImpl : powerServer : returnObj ="
                + returnObj);
        logger.debug("ServerHardwareClientImpl : powerServer : taskResource ="
                + taskResourceV2);
