...
public class ResourceDtoUtils
{
    ...
    public NetworkSets buildNetworkSetDto(final RestParams params,
            final String networkSetName, final List<String> networkNames,
            final String nativeNetworkName)
    {
        final NetworkSets dto = buildNetworkSetDto(params, networkSetName,
                networkNames);
    ...
    public Bandwidth buildBandwidth(final Double maxBandwidth,
    ...
    public LogicalInterconnectGroups buildLogicalInterconnectGroupDto(
            final RestParams params, final String logicalInterconnectGroupName,
            final HashMap<Integer, String> bayPermittedInterconnectMaps)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            interconnectMapEntryTemplateDto
                    .setLogicalLocation(logicalLocationDto);
            if (bayPermittedInterconnectMaps.get((i + 1)) != null)
            {
                interconnectMapEntryTemplateDto
                        .setPermittedInterconnectTypeUri(utils
                                .getPermittedInterconnectTypeUri(params,
            ...
            {
                interconnectMapEntryTemplateDto
                        .setPermittedInterconnectTypeUri(null);
            }
            interconnectMapEntryTemplatesDto
                    .add(interconnectMapEntryTemplateDto);
        }

        final InterconnectMapTemplate interconnectMapTemplateDto = new InterconnectMapTemplate();
        interconnectMapTemplateDto
                .setInterconnectMapEntryTemplates(interconnectMapEntryTemplatesDto);
     ...
     * }
     */
    public UplinkSet buildUplinkSetDto(final RestParams params, final String ligName,
            final String uplinkSetName, final String uplinkSetType,
            final List<String> networkNames,
            final HashMap<Integer, List<String>> bayPortMap, final String lacpTimer,
            final String fcUplinkSpeed)
    {
        return buildUplinkSetDto(params, ligName, uplinkSetName, uplinkSetType,
                bayPortMap, networkNames);
    }

    public UplinkSet buildUplinkSetDto(final RestParams params, final String ligName,
            final String uplinkSetName, final String uplinkSetType,
            final HashMap<Integer, List<String>> bayPortMap, final List<String> networkNames)
    {
        ...
        for (final Entry<Integer, List<String>> entry : bayPortMap.entrySet())
        {
            ...
            final String permittedInterconnectTypeUri = utils
                    .getPermittedInterconnectTypeUriForLigBasedOnBay(params,
                            ligName, bayRelativeValue);
            final InterconnectTypes interconnectTypeDto = interconnectTypeClient
                    .getInterconnectType(params, (permittedInterconnectTypeUri
                            .substring(permittedInterconnectTypeUri
                                    .lastIndexOf("/") + 1)));
            ...
            for (int i = 0; i < portNames.size(); i++)
            {
                ...
                {
                    if (portInfo.getPortName().equalsIgnoreCase(
                            portNames.get(i)))
                    {
                ...
                if (portNumber == -1)
                {
                    throw new SDKInvalidArgumentException(
                            SDKErrorEnum.invalidArgument, null, null, null,
                            SdkConstants.UPLINKSET
                ...
                logicalPortConfigInfo
                        .setDesiredSpeed(LogicalPortConfigInfo.DesiredSpeed.Auto);
        ...
        if (uplinkSetType.equalsIgnoreCase(SdkConstants.ETHERNET))
        {
            ...
            uplinkSetDto.setNetworkUris(utils.getNetworkUris(params,
                    networkNames));
        }
        else if (uplinkSetType.equalsIgnoreCase(SdkConstants.FIBRE_CHANNEL))
        {
            ...
            uplinkSetDto.setNetworkUris(utils.getFcNetworkUris(params,
                    networkNames));
        }
    ...
    public ProfileConnectionV3 buildProfileConnection(final RestParams params,
            final Integer j, final String networkName,
            final String requestedMbps, final Integer allocatedMbps,
            final Integer maximumMbps,
            final ProfileConnectionV3.FunctionType functionType,
            final BootControl bootControl)
    {
        ...
        {
            connection.setNetworkUri(utils
                    .getNetworkUri(params, networkName));
        }
        else if (functionType.toString().equalsIgnoreCase("FibreChannel"))
        {
            connection.setNetworkUri(utils.getFcNetworkUri(params,
                    networkName));
        }
    ...
    public SanStorage buildSanStorage(final RestParams params,
    ...
    public VolumeAttachment buildVolumeAttachment(final RestParams params,
            final String volumeName,
            final Boolean useBayNameForServerHardwareUri, final Integer j,
            final Boolean isEnabled, final List<String> storageTargets,
            final StorageTargetType storageTargetType, final String lunType,
            final HashMap<String, Integer> fcId)
    {
        final Boolean volumeIsSharable = utils
                .isVolumeSharable(params, volumeName);
        if (volumeIsSharable || !(useBayNameForServerHardwareUri))
        {
            ...
            volumeAttachment.setVolumeUri(utils.getVolumeUri(params,
                    volumeName));
            volumeAttachment.setVolumeStoragePoolUri(utils
                    .getStoragePoolFromVolume(params, volumeName));
            volumeAttachment.setVolumeStorageSystemUri(utils
                    .getStorageSystemFromVolume(params, volumeName));
            ...
            {
                if (fcId.containsKey(storageTargetPortCollectionDto
                        .getMembers().get(k).getExpectedNetworkName()))
                {
                    final StoragePath storagePath = new StoragePath();
                    storagePath.setIsEnabled(isEnabled);
                    storagePath.setConnectionId(fcId
                            .get(storageTargetPortCollectionDto.getMembers()
                                    .get(k).getExpectedNetworkName()));
    ...
    public ServerProfile buildServerProfile(final RestParams params,
            final String profileName, final String serverHardwareName,
            final Boolean useBayNameForServerHardwareUri,
            final String enclosureGroupName,
            final ServerProfile.ProfileAffinity affinity,
            final ServerProfile.AssignmentType wwnType,
            final ServerProfile.AssignmentType macType,
            final ServerProfile.AssignmentType serialNumberType,
            final SanStorage sanStorage,
            final List<ProfileConnectionV3> connections,
            final LocalStorage localStorage, final Boot boot, final Bios bios,
            final Firmware firmware)
    {
        ...
        {
            final String serverHardwareTypeUri = utils.getServerHardwareTypeUri(
                    params, serverHardwareName);
            serverProfileDto
                    .setServerHardwareTypeUri((serverHardwareTypeUri != null && serverHardwareTypeUri
                            .length() != 0) ? serverHardwareTypeUri : null);
        ...
        {
            final String serverHardwareUri = utils.getServerHardwareUri(params,
                    serverHardwareName);
            serverProfileDto
                    .setServerHardwareUri((serverHardwareUri != null && serverHardwareUri
                            .length() != 0) ? serverHardwareUri : null);
        }
        serverProfileDto.setEnclosureGroupUri(utils.getEnclosureGroupUri(
                params, enclosureGroupName));
